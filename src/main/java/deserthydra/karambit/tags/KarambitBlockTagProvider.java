...
public class KarambitBlockTagProvider extends FabricTagProvider.BlockTagProvider {
    ...
    public void configure(RegistryWrapper.WrapperLookup registries) {
        addStems(KarambitBlockTags.ROSEWATER_STEMS, KarambitBlocks.ROSEWATER);
        addStems(KarambitBlockTags.AZURITE_STEMS, KarambitBlocks.AZURITE);
    }

    private void addStems(TagKey<Block> stemTag, WoodBlocks woodBlock) {
        FabricTagBuilder stemBuilder = getOrCreateTagBuilder(stemTag);
        stemBuilder
                .add(woodBlock.stem)
